...
# Fetch the data.
df = pd.read_csv("https://ourworldindata.org/explorers/countries-in-conflict-data.csv?v=1&csvType=full&useColumnShortNames=true&Measure=Conflict+deaths&Conflict+type=All+armed+conflicts&Conflict+sub-type=Across+all+sub-types", storage_options = {'User-Agent': 'Our World In Data data fetch/1.0'})

# Fetch the metadata
metadata = requests.get("https://ourworldindata.org/explorers/countries-in-conflict-data.metadata.json?v=1&csvType=full&useColumnShortNames=true&Measure=Conflict+deaths&Conflict+type=All+armed+conflicts&Conflict+sub-type=Across+all+sub-types").json()
url_datos = (
